...
int16_t sensirion_uart_open() {
    ...
    if (HAL_UART_GetState(&huart5) == HAL_UART_STATE_READY) {
        ...
        HAL_UART_Transmit(&huart3, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY); // HAL_MAX_DELAY para esperar hasta que la transmisión esté completa
        return 0; // Éxito
    } else {
...
void sensirion_sleep_usec(uint32_t useconds) {
    // Utilizar una función de delay que se ajuste a la precisión deseada. HAL_Delay trabaja en milisegundos.
