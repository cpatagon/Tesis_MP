...
void SPS30_StartMeasurement(void) {
    ...
    snprintf(respuestaStr, sizeof(respuestaStr), MENSAJE_SIZE_RESPUESTA,
             longRespuesta);  // Formatea el mensaje de longitud
 ...
 * A pesar de la detención, los datos previamente medidos se mantienen hasta que el sensor sea
 * activado nuevamente.
 */
void SPS30_StopMeasurement(void) {
    // Variables
    uint8_t stopCmd[] =
        SPS30_FRAME_STOP_MEASUREMENT; // Define el comando específico para detener la medición.
    ...
    uart_send_receive(
        stopCmd, sizeof(stopCmd), dataBuf,
        BUFFER_SIZE_STOP_MEASUREMENT); // Envía el comando de detención y espera la respuesta.
    HAL_Delay(DELAY_STOP_MEASUREMENT); // Opcional: Retraso para asegurar que el sensor procese el
    ...
    int longRespuesta = SHDLC_CalculateDataSize(
        dataBuf, sizeof(dataBuf));           // Calcula la longitud de la respuesta útil.
    char respuestaStr[BUFFER_SIZE_RESPONSE]; // Prepara un buffer para el mensaje de longitud de
                                             // respuesta.
    snprintf(respuestaStr, sizeof(respuestaStr), MENSAJE_SIZE_RESPUESTA,
             longRespuesta);  // Formatea el mensaje con la longitud de la respuesta.
    uart_print(respuestaStr); // Imprime la longitud de la respuesta.
}
...
void SPS30_ReadData(void) {
    ...
    uint8_t originalData[BUFFER_SIZE_READ_DATA]; // Buffer para almacenar los datos tras revertir
                                                 // ByteStuffing
    ...
    uart_send_receive(readCmd, sizeof(readCmd), dataBuf,
                      BUFFER_SIZE_READ_DATA); // Envía comando y recibe datos
    uart_print(MSG_RESPUESTA);
...
void SPS30_SerialNumber(void) {
    ...
    uint8_t dataBuf[BUFFER_SIZE_SERIAL_NUMBER]; // Asegúrate de que este buffer sea adecuado para
                                                // los datos
    memset(dataBuf, CLEAR_VAR, sizeof(dataBuf));
    uart_print(MSG_SOLICITAR);
    uart_vector_print(sizeof(readCmd), readCmd);
